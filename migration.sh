...
# CHANGEABLES
export SITE_ENV="brookgreend8.dev"
export PANTHEON_D7_SITE="brookgreen"
...
export D7_MYSQL_URL=$(terminus connection:info $PANTHEON_D7_SITE.$PANTHEON_D7_BRANCH --field=mysql_url)
terminus drush $SITE_ENV -- migrate-upgrade --configure-only --legacy-db-url=$D7_MYSQL_URL --legacy-db-key=upgrade --legacy-root=$PANTHEON_D7_URL
